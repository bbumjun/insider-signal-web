      ...
      {activities.map((activity, i) => (
        <div key={i} className="p-4 hover:bg-white/5 transition-colors group cursor-pointer">
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className={cn(
                ...
                activity.type === 'insider' 
                  ? activity.data.transactionCode === 'P' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
            ...
            {activity.type === 'insider' ? (
              activity.data.transactionCode === 'P' ? <ArrowUpCircle className="w-4 h-4 text-emerald-400" /> : <ArrowDownCircle className="w-4 h-4 text-red-400" />
            ) : (
